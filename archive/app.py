...
# Shared variable for the listening state (thread-safe)
listening_flag = {"value": False}

# Constants
POLLING_DELAY = 0.5  # Delay for polling listening state
...
if st.button("Start/Stop Recording"):
    ...
    listening_flag["value"] = st.session_state.listening
...
def voice_assistant():
    while True:
        if listening_flag["value"]:  # Use thread-safe flag
            try:
                # Record audio
                ...
                if not user_input:
                    logging.warning("No transcription detected. Please try again.")
                    continue
